...
export class ProductService {
  ...
  constructor(private http: HttpClient, private productCatalogService: ProductCatalogService) { }

  loadProductsByCategory() {
    this.dataStore  = { products: [] };
    this.productsByCategory.next(Object.assign({}, this.dataStore).products);

    this.productCatalogService.productCatalogSelectedAction$.pipe(
      ...
      switchMap(async (productCatalog) => {
        const params = new HttpParams().set('productCatalogId', productCatalog.id.toString());
        this.http.get<Product[]>(apiUrl, {headers, params}).subscribe(
          products => {
  ...
  createProduct(productCatalogId: number, product: Product): void {
    const params = new HttpParams().set('productCatalogId', productCatalogId.toString());
    this.http.post<Product>(apiUrl, JSON.stringify(product), {headers, params}).subscribe(
        product => {
  ...
  updateProduct(product: Product): void {
    this.http.put<Product>(`${apiUrl}/${product.id}`, JSON.stringify(product), {headers}).subscribe(
      product => {
